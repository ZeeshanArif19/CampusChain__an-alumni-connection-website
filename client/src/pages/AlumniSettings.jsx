...
import { FaUserCog, FaBell, FaUserShield, FaDownload, FaTrash, FaSignOutAlt } from 'react-icons/fa';
...
const AlumniSettings = () => {
  ...
  const handleDownloadData = () => {
    const data = {
      ...
      privacy,
      activity: [
        'Joined Alumni Meetup 2025',
        'Connected with Ravi Patel',
    ...
    a.href = url;
    a.download = 'my-campuschain-alumni-data.json';
    a.click();
  ...
  return (
    <DashboardLayout role="alumni">
      <div className="max-w-2xl mx-auto space-y-10 py-8">
        {/* Page Title */}
        <div className="mb-2">
          <h1 className="text-3xl font-bold text-indigo-700 mb-1 flex items-center gap-2"><FaUserCog className="text-indigo-500" /> Alumni Settings</h1>
          <p className="text-gray-500">Manage your alumni account, privacy, and preferences</p>
        </div>
            ...
            <label className="flex items-center gap-3 justify-between">
              ...
              <span className="inline-block">
                <button
                  ...
                  className={`w-8 h-8 rounded-md flex items-center justify-center transition-colors duration-200 border-2 focus:outline-none focus:ring-2 focus:ring-green-400 ${notifications.email ? 'bg-green-600 border-green-700 text-white' : 'bg-gray-700 border-gray-800 text-gray-300'}`}
                  title={notifications.email ? 'Enabled' : 'Disabled'}
                >
                  {notifications.email ? <span className="text-lg">✓</span> : <span className="text-lg">✕</span>}
                </button>
              </span>
            </label>
            <label className="flex items-center gap-3 justify-between">
              ...
              <span className="inline-block">
                <button
                  ...
                  className={`w-8 h-8 rounded-md flex items-center justify-center transition-colors duration-200 border-2 focus:outline-none focus:ring-2 focus:ring-green-400 ${notifications.sms ? 'bg-green-600 border-green-700 text-white' : 'bg-gray-700 border-gray-800 text-gray-300'}`}
                  title={notifications.sms ? 'Enabled' : 'Disabled'}
                >
                  {notifications.sms ? <span className="text-lg">✓</span> : <span className="text-lg">✕</span>}
                </button>
              </span>
            </label>
            <label className="flex items-center gap-3 justify-between">
        ...
        {/* Privacy Controls */}
        <div className="bg-gradient-to-br from-white via-indigo-50 to-purple-50 rounded-2xl shadow-lg p-8 border border-indigo-100">
          <h2 className="text-xl font-bold text-indigo-700 mb-4 flex items-center gap-2"><FaUserShield className="text-indigo-400" /> Privacy Controls</h2>
          <div className="space-y-5">
            <label className="flex items-center gap-3 justify-between">
              ...
              <span className="inline-block">
                <button
                  type="button"
                  aria-pressed={privacy.profileVisible}
                  onClick={() => setPrivacy(prev => ({ ...prev, profileVisible: !prev.profileVisible }))}
                  className={`w-8 h-8 rounded-md flex items-center justify-center transition-colors duration-200 border-2 focus:outline-none focus:ring-2 focus:ring-green-400 ${privacy.profileVisible ? 'bg-green-600 border-green-700 text-white' : 'bg-gray-700 border-gray-800 text-gray-300'}`}
                  title={privacy.profileVisible ? 'Visible' : 'Hidden'}
                >
                  {privacy.profileVisible ? <span className="text-lg">✓</span> : <span className="text-lg">✕</span>}
                </button>
              </span>
            </label>
            <label className="flex items-center gap-3 justify-between">
              <span>Allow direct messages from other alumni</span>
              <span className="inline-block">
                <button
                  type="button"
                  aria-pressed={privacy.allowMessages}
                  onClick={() => setPrivacy(prev => ({ ...prev, allowMessages: !prev.allowMessages }))}
                  className={`w-8 h-8 rounded-md flex items-center justify-center transition-colors duration-200 border-2 focus:outline-none focus:ring-2 focus:ring-green-400 ${privacy.allowMessages ? 'bg-green-600 border-green-700 text-white' : 'bg-gray-700 border-gray-800 text-gray-300'}`}
                  title={privacy.allowMessages ? 'Allowed' : 'Blocked'}
                >
                  {privacy.allowMessages ? <span className="text-lg">✓</span> : <span className="text-lg">✕</span>}
                </button>
              </span>
            </label>
          </div>
        ...
        <div className="bg-gradient-to-br from-white via-indigo-50 to-purple-50 rounded-2xl shadow-lg p-8 border border-indigo-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-500">Export your profile and activity as a JSON file</p>
          </div>
          <button
        ...
        <div className="bg-gradient-to-br from-white via-red-50 to-indigo-50 rounded-2xl shadow-lg p-8 border border-red-100">
          ...
          {showDelete && (
            <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-lg">
              <p className="text-red-700 mb-2 font-semibold">Are you sure you want to delete your account? This action cannot be undone.</p>
