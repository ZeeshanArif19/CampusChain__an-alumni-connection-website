...
const initialProfile = {
  _id: null,
  name: 'Bhavya Kapoor',
  headline: 'Aspiring Software Engineer | B.Tech CSE, IIT Bombay',
  about: 'Passionate about technology, coding, and building impactful solutions. Seeking opportunities to learn and grow in the tech industry.',
  email: 'bhavya.kapoor@email.com',
  phone: '',
  graduationYear: '',
  branch: '',
  courseName: '',
  ...
  profilePhoto: '',
  education: [
    { college: 'IIT Bombay', course: 'B.Tech Computer Science', year: '2025' },
  ],
  skills: ['JavaScript', 'React', 'Python', 'Teamwork'],
  projects: [],
  certifications: [],
  languages: [],
...
function getProfileCompleteness(profile) {
  if (!profile) return 0;
  let fields = [profile.name, profile.headline, profile.about, profile.education.length, profile.skills.length, profile.projects.length, profile.certifications.length, profile.languages.length, profile.achievements.length, profile.experience.length];
  let filled = fields.filter(f => f && f !== '' && (Array.isArray(f) ? f > 0 : true)).length;
  return Math.round((filled / fields.length) * 100);
}

const StudentProfile = () => {
const [profile, setProfile] = useState(initialProfile); // Initialize with default profile
...
const [isLoading, setIsLoading] = useState(true);
 const [isSubmitting, setIsSubmitting] = useState(false);
  ...
  // Sync profile from backend by email on mount
useEffect(() => {
    const fetchProfile = async () => {
      setIsLoading(true);
      const email = profile.email || localStorage.getItem('userEmail') || '';
      console.log('useEffect triggered - Fetching profile for email:', email);

      if (email && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        try {
          const res = await axios.get(`${API_BASE_URL}/student/get/${email}`);
          setProfile({ ...res.data, email }); // Persist email
          setIsExisting(true);
  ...
  // Ensure profile has all required properties (but allow empty name during editing)
  if (!profile) {
    return (
      <DashboardLayout role="student">
        <div className="max-w-4xl mx-auto bg-white rounded-xl shadow p-8 mt-8">
  ...
  try {
    ...
    console.log('Saving profile for:', profile.email);

    if (isExisting) {
      await axios.put(`${API_BASE_URL}/update-by-email/${profile.email}`, profile);
      alert('Profile updated successfully!');
    } else {
      await axios.post(`${API_BASE_URL}/create`, profile);
...
const handleSubmit = async () => {
    console.log('handleSubmit triggered'); // Debug: Confirm button click
    if (isSubmitting) {
    ...
    try {
      ...
      console.log('Email used for update:', email);
      const { _id, ...profileData } = profile;
      console.log('Profile data sent:', profileData);

      // Try update first
      try {
        const response = await axios.put(`${API_BASE_URL}/student/update-by-email/${email}`, profileData);
        alert('Profile updated successfully');
        setIsExisting(true);
        console.log('Update response:', response.data);
      } catch (updateErr) {
        console.error('Update error:', updateErr.response?.status, updateErr.response?.data || updateErr.message);
        if (updateErr.response?.status === 404) {
          // Profile not found, try create
          const response = await axios.post(`${API_BASE_URL}/student/create`, profileData);
          alert('Profile created successfully');
          setIsExisting(true);
  ...
  const handleDownloadPDF = () => {
    // Create a new window with the profile content
      ...
      <html>
        ...
        <body>
          <div class="header">
            ...
            <p><strong>${profile.headline || 'Student Headline'}</strong></p>
            <p>${profile.email}</p>
          </div>
          ...
          <div class="section">
            ...
            <div class="contact-info">
              ...
              <div><strong>Year Type:</strong> ${profile.yearType || 'Not provided'}</div>
              ${profile.yearType === 'Graduate' ? `<div><strong>Graduation Year:</strong> ${profile.graduationYear || 'Not provided'}</div>` : ''}
            </div>
          </div>
        </body>
      </html>
    ...
    <DashboardLayout role="student">
      <div className="max-w-4xl mx-auto bg-white rounded-xl shadow p-0 mt-8 overflow-hidden">
        ...
        <div className="pt-20 px-8 pb-8 space-y-8">
          {/* Name, Headline, Social Links */}
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-2">
          <div>
              <h2 className="text-3xl font-bold text-gray-800">{editMode ? <input name="name" value={profile.name || ''} onChange={handleChange} className="font-bold text-2xl border-b border-indigo-200 focus:outline-none" placeholder="Enter your name" /> : (profile.name || 'Name not provided')}</h2>
              <p className="text-lg text-gray-600 mt-1">{editMode ? (<input name="headline" value={profile.headline || ''} onChange={handleChange} className="w-full border-b border-indigo-200 focus:outline-none" placeholder="Enter your headline" />) : (profile.headline || 'Headline not provided')}</p>
          ...
          <div className="bg-indigo-50 rounded-lg p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <h4 className="font-semibold text-gray-700 mb-1">Year</h4>
                {editMode ? (
                  <select name="yearType" value={profile.yearType} onChange={handleChange} className="w-full border-b border-indigo-200 focus:outline-none">
                    <option value="">Select</option>
                    <option value="Graduate">Graduate</option>
                    <option value="Fresher">Fresher</option>
                  </select>
                ) : (
                  <p className="text-gray-600">{profile.yearType || <span className="text-gray-400">Not provided</span>}</p>
                )}
          </div>
          {profile.yearType === 'Graduate' && (
            <div>
          ...
          <div className="flex justify-end gap-2 mt-6">
            ...
            {editMode ? (
              <>
                <button type="button" className="bg-indigo-500 text-white px-6 py-2 rounded-lg font-semibold hover:bg-indigo-600" onClick={handleSubmit}>Save</button>
